...
int main (int argc, char **argv)
{
  ...
  dungeon[2].contents |= GOLD; // Put a piece of gold in room 2
  dungeon[8].contents |= EXIT; // Put exit in room 8
  dungeon[4].contents |= SPHINX; // Put the sphinx in room 4
  dungeon[6].contents |= SWORD; // Put the sword in room 6
  now = &dungeon[0]; // Set the starting room
  quest(&dungeon[0],ROOMS); // Calls YOUR quest function
  if (now == &dungeon[8]) { // Check if the final move is the exit
    int done = true;
    int room = 0;
